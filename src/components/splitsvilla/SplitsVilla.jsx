...
const SplitsVilla = () => {
  ...
  const settleRecord = (record) => {
    ...
    const cost = Math.abs(record.cost).toFixed(2);
    const newUserList = userData.map((item) =>
    ...
    const updatedRecords = updateRecords(records, ownUser, currentUser);
                  ...
                  <div key={rcd.to?.id || rcd.from?.id} className="list-item">
                    {rcd.to ? (
                      <div className="depth-record">
                        ...
                        <button type="button" onClick={() => settleRecord(rcd)}>
                          Settle
                        </button>
                      </div>
                    ) : (
