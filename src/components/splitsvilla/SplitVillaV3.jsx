  ...
  const settleRecord = (record, isPartial) => {
    ...
    const cost =
      isPartial === "partial"
        ...
        : Math.abs(record.cost);
        console.log(cost,record,userData)
    const newUserList = userData.map((item) =>
      item.id === ownUser.id || item.id === currentUser.id
        ? {
            ...item,
            amount: Number(
              item.amount - (item.id === ownUser.id ? +cost : -cost)
            ),
            history: [
              ...item.history,
              {
    ...
    setCurrentUser((item) => ({
      ...item,
      amount: Number(item.amount + cost),
      history: [
        ...item.history,
        {
          settledBy: {
                ...
                <div className="amount-box">
                  ...
                  <div>
                    <input
                      ...
                      ref={payAmountref}
                      className="input-amount"
                      onKeyDown={(e) => e.keyCode === 13 && settleRecord(partialPay, "partial")}
                      // onKeyDown={handleKeyChange}
                    />
                  </div>
                </div>
